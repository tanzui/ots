...
1.  创建开通了Stream流的数据表。
    1.  在[Table Store控制台](https://ots.console.aliyun.com/)创建实例。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111913_zh-CN.png)

    2.  在该实例下创建数据表，并且开启数据表的Stream功能。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111914_zh-CN.png)

2.  创建函数计算的函数。
    1.  在[函数计算控制台](https://fc.console.aliyun.com/#/serviceList/)创建服务（Service）。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111915_zh-CN.png)
    ...
    2.  在Service的高级配置中，您可以配置服务的角色，用于授权函数进行日志收集，以及在计算函数中继续访问用户的其他资源。详细信息请参见[函数计算权限模型](https://www.alibabacloud.com/help/doc-detail/52885.htm)。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111916_zh-CN.png)
    ...
    6.  配置函数信息。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111918_zh-CN.png)
...
3.  创建和测试Table Store触发器。
    ...
    2.  创建过程中需要授权Table Store发送事件通知所需的权限。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111919_zh-CN.png)
...
4.  当使用 `records=json.loads(event)` 测试OK之后，可以修改为 `records = cbor.loads(event)` 并保存，当Table Store中有数据写入时，相关的函数逻辑就会触发。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/20324/153803538111920_zh-CN.jpg)
