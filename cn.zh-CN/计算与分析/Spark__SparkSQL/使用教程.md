...
**说明：** 表格中空白的部分不需要写入，因为表格存储是一个 schema-free 的存储结构（[数据模型](../../../../cn.zh-CN/隐藏/前言.md#)），没有值也不需要写入`NULL`。
...
-   TBLPROPERTIES
    -   tablestore.endpoint（必填）：访问表格存储的[服务地址](../../../../cn.zh-CN/开发指南/基础概念/服务地址.md#)，也可以在表格存储控制台上查看这个实例的 endpoint 信息。

    -   tablestore.instance（可选）：表格存储的[实例](../../../../cn.zh-CN/开发指南/基础概念/实例.md#)名。若不填，则为 tablestore.endpoint 的第一段。
    ...
    -   tablestore.sts\_token（可选），请参见[授权管理](../../../../cn.zh-CN/开发指南/授权管理/RAM 和 STS 介绍.md#)。
